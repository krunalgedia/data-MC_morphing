#pplied_res_2018-06-27_t_dataset_sig_test_new_NN.h5i # Characterize FFWD NN output
#load libs
import keras.models
import os
...
parser = OptionParser(option_list=[
    ...
    make_option("--inp-file",type='string',dest='inp_file',default='applied_res_2018-08-08_t_dataset_sig_test_plots_NN.h5'),
...
with open('/mnt/t3nfs01/data01/shome/krgedia/Workspace/lxplus/qreg_pt_norm/config_2016_updated_MC_corr.json') as fin:
...
data = data.assign(newNNreg_corr=y_pred[:,0])
data = data.rename(columns={'newNNreg_corr':'Jet_pt_reg_NN_corr'})
if res is not None :
    data = data.assign(newNNreg_res_corr=res)
    data = data.rename(columns={'newNNreg_res_corr':'Jet_resolution_NN_corr'})
